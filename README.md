...
- 🎯 Entry points to execute your program using `python -m <pdf_coordinates_learner>` or `$ pdf_coordinates_learner` with basic CLI argument parsing.
...
---
# pdf_coordinates_learner

[![codecov](https://codecov.io/gh/pearsonk27/pdf-coordinates-learner/branch/main/graph/badge.svg?token=pdf-coordinates-learner_token_here)](https://codecov.io/gh/pearsonk27/pdf-coordinates-learner)
[![CI](https://github.com/pearsonk27/pdf-coordinates-learner/actions/workflows/main.yml/badge.svg)](https://github.com/pearsonk27/pdf-coordinates-learner/actions/workflows/main.yml)

Awesome pdf_coordinates_learner created by pearsonk27

## Install

from source
```bash
git clone https://github.com/pearsonk27/pdf-coordinates-learner pdf_coordinates_learner
cd pdf_coordinates_learner
make install
```
...
```bash
pip install pdf_coordinates_learner
```

## Executing

```bash
$ pdf_coordinates_learner run --port 8080
```

or

```bash
python -m pdf_coordinates_learner run --port 8080
```

or

```bash
$ uvicorn pdf_coordinates_learner:app
```

## CLI

```bash
❯ pdf_coordinates_learner --help
Usage: pdf_coordinates_learner [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ pdf_coordinates_learner create-user --help
Usage: pdf_coordinates_learner create-user [OPTIONS] USERNAME PASSWORD
...
```bash
pdf_coordinates_learner create-user admin admin --superuser
```
...
Run with `pdf_coordinates_learner run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `pdf_coordinates_learner create-user`.
...
-----------------------------------------------------
pdf_coordinates_learner/__init__.py              4      0   100%
pdf_coordinates_learner/app.py                  16      1    94%
pdf_coordinates_learner/cli.py                  21      0   100%
pdf_coordinates_learner/config.py                5      0   100%
pdf_coordinates_learner/db.py                   10      0   100%
pdf_coordinates_learner/models/__init__.py       0      0   100%
pdf_coordinates_learner/models/content.py       47      1    98%
pdf_coordinates_learner/routes/__init__.py      11      0   100%
pdf_coordinates_learner/routes/content.py       52     25    52%
pdf_coordinates_learner/routes/security.py      15      1    93%
pdf_coordinates_learner/routes/user.py          52     26    50%
pdf_coordinates_learner/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in pdf_coordinates_learner/default.toml.
